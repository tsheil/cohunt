...
| **AI IDE supply chain + DXT** | Project file exploitation (30+ vulns, 24 CVEs), extension squatting, Chromium flaws, rules file backdoor (invisible Unicode), workspace trust bypass, **Claude DXT zero-click RCE** (CVSS 10.0, confused deputy chains, AppleScript injection), Blackbox AI RCE via PNG, **plugin hook injection** (PromptArmor — stderr → prompt injection → codebase exfil) | ai-hunting/reference/ide-supply-chain.md |
...
| **Enterprise AI + AI-as-exfil** | GeminiJack (zero-click via shared docs), Reprompt (URL param injection → persistent control), CRM form injection (ForcedLeak CVSS 9.4), **Agentic Collapse** (CVE-2026-22200 — chat with AI agent → semantic coercion → legacy tool RCE, WAF-invisible), AI-as-exfil-channel (attacker content → AI agent → data exfiltration; test all paths where untrusted data enters AI context with privileged tools) | ai-hunting/reference/agent-attack-patterns.md |
...
| **File upload & processing chain** | Full chain: upload bypass (U+200B CVE-2026-28289 CVSS 10.0), archive extraction (Zip Slip), parser abuse (ImageMagick/ffmpeg/wkhtmltopdf SSRF/RCE), import-from-URL SSRF, signed URL manipulation, cross-tenant file access, **AI/LLM file injection** (RAG poisoning, EXIF/PDF/DOCX prompt injection, multimodal vision injection — highest-growth file attack surface) | file-processing/SKILL.md |
...
| **Windows/infra + firewall mgmt** | MotW bypass chain (3 in Feb 2026 Patch Tuesday, APT28), SSRF chains, critical infra auth bypass, Chrome sandbox escape, March PT 79-83 CVEs including SQL Server sysadmin zero-day + **Excel info disclosure via Copilot Agent mode** (CVE-2026-26144 Critical), **firewall management consoles** (boot-time auth bypass + Java deser RCE, Cisco FMC dual CVSS 10.0) | infrastructure-security/reference/infrastructure-vulns.md |
| **React RSC / Next.js** | Deserialization in Flight protocol (React2Shell); nation-state exploitation deploying K8s backdoors (MINOCAT/SNOWLIGHT/COMPOOD) | vuln-patterns SKILL.md |
| **Salesforce Experience Cloud** | Guest user profile misconfig → unauthenticated CRM data via `/s/sfsites/aura` (ShinyHunters ~400 orgs March 2026); test with AuraInspector | auth-testing SKILL.md |
| **Repos with AI tooling** | .claude/ hooks RCE (CVE-2025-59536), .mcp.json rogue servers, .cursorrules injection, API key redirect (CVE-2026-21852) | source-code-audit SKILL.md |
| **Internal service exposure** | Management services externally accessible (Juniper CVE-2026-21902 CVSS 9.3), appliance On-Box features pre-auth | infrastructure-security/reference/infrastructure-vulns.md |
| **K8s / container security** | IngressNightmare, RBAC escalation, IMDS credential theft, Kyverno namespace escape (CVSS 10.0), runC escape, Istio mTLS bypass | cloud-security/reference/kubernetes-security.md |
| **Multi-cloud / federation** | Workload identity federation abuse, cross-cloud credential chains, Terraform state exposure, stale federation trusts | cloud-security/reference/multi-cloud-pivots.md |
...
- **No disclosures**: adjust duplicate risk down. **No bounty program**: suggest VDP. **High activity** (97+ researchers): focus on logic bugs + chains. **New program** (<6 months): opportunity, but verify response times. **Strong WAF/CSP**: weight toward logic bugs. **Web + mobile in scope**: prioritize shared API backends (one vuln, broader impact). **Time budget**: strict prioritization. **Specialization mentioned**: weight plan accordingly. **Return visit**: reference previous findings, focus on untested areas + new features.
