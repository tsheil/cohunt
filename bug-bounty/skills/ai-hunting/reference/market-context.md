...
- **Claude Code Plugin Hook Injection** (PromptArmor, March 2026): marketplace plugins can hijack Claude Code agent via hook system — using `UserPromptSubmit`/`PreToolUse` hooks to rewrite permissions and auto-approve commands, plus injected plugin commands for exfiltration. **Hunter implication:** any AI tool with plugin/extension hooks that feed back into model context is vulnerable to hook-based prompt injection; test all plugin feedback channels for source attribution and filtering
