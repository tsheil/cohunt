...
- **MCP OAuth account takeover**: CSRF-style attacks on Remote MCP servers (Obsidian Security); MCP authorization is **optional** per spec — when implemented over HTTP, follows OAuth 2.1 + PKCE + RFC 8707 Resource Indicators; 38% of servers skip auth entirely
...
- **CVE-2026-30903 Zoom Workplace Mail** (March 11, 2026, CVSS 9.6): External Control of File Name/Path in Zoom Workplace for Windows Mail feature — network-exploitable, requires user interaction (UI:R); path control may allow EoP. Affects < v6.6.0. **Hunter implication:** desktop collaboration tool mail/attachment handlers are a systematic attack surface for file path control → EoP chains; test mail processing in Zoom, Teams, Slack, Webex for similar path manipulation
...
- **CVE-2026-22200 "Agentic Collapse"** (Penligent/Horizon3.ai, March 2026): osTicket PHP filter chain → arbitrary file read + potential RCE; when connected to AI RAG agent, attacker chats with public AI support and the agent executes the vulnerable tool via "semantic coercion" — no login, no HTTP exploit, WAF-invisible. Pattern: any vulnerable tool accessible to an AI agent becomes exploitable through natural language. **Hunter implication:** test AI agent→legacy tool chains where the agent bridges unauthenticated chat to authenticated internal tools; traditional file-read/RCE vulns in agent-connected backends become zero-interaction exploits
- **ShadowLeak + ZombieAgent ChatGPT zero-click** (Radware, Sept 2025–Jan 2026): ShadowLeak — crafted email triggers ChatGPT Deep Research agent to exfil Gmail data with zero user action. ZombieAgent — static pre-constructed URLs exfil data one character at a time, bypassing URL-modification defenses. Both reported via Bugcrowd, fixed by OpenAI. **Hunter implication:** AI agents with connected services (email, calendar, docs) are prime targets for zero-click exfil via poisoned content in those services; test agent-connected accounts for data leakage triggered by malicious emails/docs/calendar events
- **SecureClaw by Adversa AI** (Feb 16, 2026): first OWASP-aligned open-source security plugin for OpenClaw agents; 56 audit checks, 5 hardening modules, 3 background monitors; two-layer defense (code-level plugin + behavioral skill). Full OWASP Agentic Top 10 + CoSAI + MITRE ATLAS coverage. **Hunter implication:** OpenClaw agents with SecureClaw deployed are hardened — test for gaps between audit checks; agents WITHOUT SecureClaw (majority) remain soft targets
- **CAI (Cybersecurity AI) framework** (Alias Robotics, 2026): open-source agentic pentesting framework; 300+ AI models, built-in recon/exploit/privesc tools; 1st place AI team in HTB "AI vs Human" CTF. Apache 2.0 licensed. **Hunter implication:** open-source pentesting AIs democratize automated hunting — differentiate with business logic, multi-tenant, and chain-building skills that tool-based automation can't replicate
- **Penligent agentic pentesting** (2026): autonomous AI hacker — one-click access to 200+ Kali tools, feedback-loop exploit refinement, generates PoC scripts, adjusts plans on environmental changes. Operates in full autonomy mode (not just scanning). **Hunter implication:** Penligent + XBOW + CAI represent the agentic pentesting convergence — commodity vuln classes are fully automatable; human hunters must focus on logic bugs, access control reasoning, and attack chains requiring business context
- **Hono cookie/SSE injection** (CVE-2026-29086 + CVE-2026-29085, March 2026): unsanitized semicolons/CR/LF in Hono framework cookie and SSE response handling; session fixation via cookie injection, SSE field injection. **Hunter implication:** edge-first frameworks (Hono, Bun, Deno) have immature security surfaces — test all edge runtime apps for header/response injection patterns
- **Codex GPT-5.4 review quality** (March 2026): GPT-5.4 with extra-high reasoning provided highly actionable codebase review — identified stale claims, structural gaps, and missing attack surfaces with external source verification. **Hunter implication:** use frontier models (GPT-5.4, Claude Opus) for codebase audit with explicit source verification requirements; reasoning models catch staleness and accuracy issues that pattern-matching misses
- **LeakyLooker** (Tenable, March 10, 2026): 9 cross-tenant vulnerabilities in Google Looker Studio across multiple distinct issue types — stored-credential inheritance in PostgreSQL-like connectors, Linking API abuse for BigQuery/Spanner cross-project queries, and browser-proxied exfiltration via shared reports. Reported to Google starting June 2025; TRAs published Sept-Oct 2025; all fixed. **Hunter implication:** cloud analytics/BI tools (Looker Studio, Power BI, Tableau Cloud, Metabase) that share reports with embedded service credentials are systematic cross-tenant attack surface — test report cloning, sharing, and embedding for credential inheritance
- **Equixly agentic pentesting benchmark** (March 2026, vendor-claimed): Equixly's Agentic AI Hacker claims **230 vulnerabilities in 1 hour** across 30 challenges; 15 human pentesters solved 14/30 in 2+ hours; claims **80% more issues than ZAP DAST** and 8 high-severity vulns DAST missed; 43% of tested popular MCP servers had command injection. **Hunter implication:** if vendor claims hold, AI pentesting tools are outperforming traditional DAST — human competitive advantage remains business context, multi-step chain reasoning, and novel attack patterns
- **Microsoft March 2026 Patch Tuesday** (March 10, 2026): 79-84 CVEs (varies by counting methodology), 8 Critical; includes SharePoint RCE pair (CVE-2026-26106 + CVE-2026-26114, both require auth/PR:L, CVSS 8.8), SQL Server EoP to sysadmin (CVE-2026-21262, publicly disclosed zero-day), AFD.sys EoP (CVE-2026-24293), AD Unicode SPN collision EoP (CVE-2026-25177, CVSS 8.8). **Hunter implication:** on-prem Microsoft infrastructure has a cluster of chainable vulns; AD SPN collision is a novel EoP vector — test Unicode character handling in identity management systems
- **XBOW $75M Series B** (June 2025, market context active March 2026): Altimeter Capital led, Sequoia + NFDG participated; $117M total funding. XBOW reached #1 on HackerOne US leaderboard with 1,000+ valid reports, zero false positives. **Hunter implication:** XBOW dominates commodity vuln classes; human competitive edge is business logic, multi-tenant isolation, and chain-building requiring domain context
- **MCP attack surface metrics** (multiple sources): 5.5% of 1,899 open-source MCP servers contain tool poisoning (Queen's University, June 2025); 30 MCP CVEs filed in 60 days (Adversa AI, March 2026); 38% of 500+ scanned servers lack authentication (Prompt Security). **Hunter implication:** MCP tool poisoning + unauthenticated access are the two highest-hit-rate MCP attack surfaces; enumerate all accessible servers and test for both
- **CISA KEV March 2026 batch** (March 3-9, 2026): 10 CVEs added across 3 alerts (March 3: 2, March 5: 5 older CVEs, March 9: 3). Key recent additions: CVE-2026-21385 (Qualcomm), CVE-2026-22719 (VMware Aria cmd injection), CVE-2026-1603 (Ivanti EPM auth bypass), CVE-2025-26399 (SolarWinds WHD 3rd deser bypass), CVE-2021-22054 (Omnissa SSRF). **Hunter implication:** Ivanti/SolarWinds continue accumulating KEV entries — test enterprise management consoles for variant bypasses of previously-patched auth/deser flaws
- **MCP denial-of-wallet attacks** (March 2026): malicious MCP servers can induce cyclic "overthinking loops" amplifying token consumption up to 142.4x with severe latency increase; creates denial-of-wallet risk draining API budgets. **Hunter implication:** test MCP servers for unbounded tool execution chains, recursive tool calls, and missing cost/timeout controls — DoW is an emerging vuln class with growing triage acceptance
- **Plunk SNS webhook SSRF** (CVE-2026-32096, March 11 2026): unauthenticated SSRF via SNS webhook handler — attacker sends crafted request forcing server to make arbitrary outbound HTTP GET to any reachable host; affects <= 0.6.0, fixed in Plunk v0.7.0. **Hunter implication:** cloud notification webhook handlers (SNS, PubSub, Event Grid) are systematic SSRF vectors — test any endpoint accepting cloud provider notification payloads for URL validation bypass
