...
- **CISA March 2026 surge**: 10+ vulnerabilities added to KEV in three batches (March 3, 5, 9); **2 Emergency Directives active** (ED 26-03 for Cisco SD-WAN). Pace indicates accelerating exploitation of network infrastructure
...
- **XBOW academic scrutiny** (arXiv:2506.23592): paper challenges "fully autonomous" claims — all 1,060 findings required human security team review pre-submission; operates at Level 3-4 autonomy. Key distinction: automation vs. autonomy
...
- **Android March 2026**: 129 vulnerabilities patched; CVE-2026-0006 Critical System RCE in Media Codecs (no privileges, no interaction); CVE-2026-21385 Qualcomm 0-day (actively exploited, 230+ chipsets)
...
- **Microsoft March 2026 Patch Tuesday**: 67 vulnerabilities total, **4 actively exploited zero-days** — Windows kernel use-after-free → SYSTEM escalation, NTFS heap memory read via crafted USB, Microsoft Management Console exploitation; all 4 added to CISA KEV. VHD + MSC delivery vectors heavily used in phishing chains
- **CISA KEV March 9 batch**: SolarWinds WHD CVE-2025-26399 (CVSS 9.8, third deserialization bypass — Huntress observed ManageEngine RMM post-exploitation), Ivanti EPM CVE-2026-1603 (CVSS 8.6, "magic number 64" auth bypass), Omnissa Workspace ONE CVE-2021-22054 (SSRF). **Three KEV additions in one day** indicates acceleration
- **OpenAI acquires Promptfoo** (March 9, 2026): open-source LLM red-teaming CLI ($86M valuation, 125K+ devs, 30+ Fortune 500); pledged open-source continuity; signals frontier labs consolidating AI red-team tooling
- **XBOW #1 globally on HackerOne** (March 2026): 1,060 reports (54 critical, 242 high, 524 medium) in ~90 days; **85x faster** than human pentesters on 104 real-world scenarios; $75M Series B (Altimeter/Sequoia). Compute costs exceed bounty earnings — not yet profitable
- **Google GTIG zero-day report** (March 5, 2026): 90 zero-days exploited in 2025 (up from 78 in 2024); **48% targeted enterprise** (network appliances #1 target); commercial spyware vendors surpassed state actors as largest zero-day users; PRC-nexus groups used 10 zero-days; mobile zero-days rose to 15. **Hunter implication:** enterprise network appliances are highest-value targets for zero-day research
- **Bug bounty market $2.06B** (2026), projected $7.74B by 2035 at 15.94% CAGR (Business Research Insights). Alt forecast: $3.98B by 2032 at 16.3% CAGR
- **mcp-atlassian CVE-2026-27825** (CVSS 9.1): critical unauthenticated RCE + SSRF in most popular Atlassian MCP connector; 30 MCP CVEs in 60 days (Adversa AI March digest); 82% of 2,614 MCP implementations vulnerable to path traversal (Endor Labs)
...
- **97% of organizations** would consider AI pentesting (Aikido survey, 450 CISOs/AppSec engineers). 56.4% of ransomware CVEs first identified via zero-day exploitation (up from 33% in 2024; VulnCheck). Attackers exploit 28% of vulns within 1 day of CVE disclosure
- **SAP March 2026 Patch Tuesday** (March 10): 15 security notes, 2 Critical (CVSS 9.8 Log4j 1.2.17 code injection + CVSS 9.1 insecure deserialization). Enterprise portal deser targets remain high-value
- **Android March 2026 bulletin**: 129 vulnerabilities patched including critical RCE CVE-2026-0006 (CVSS 9.8) and actively exploited Qualcomm CVE-2026-21385 (234 chipsets)
...
- **Microsoft March 2026 Patch Tuesday**: 84 CVEs, 2 zero-days — ICMP RCE (CVSS 9.8, no auth/interaction) + Outlook EoP (CVSS 9.8, email auto-triggers). Network protocol handlers and email processing remain high-value targets
- **CISA KEV March 2026 wave**: VMware Aria CVE-2026-22719 (command injection, active exploitation), SolarWinds WHD CVE-2025-26399 (deser RCE CVSS 9.8), Ivanti EPM CVE-2026-1603 (auth bypass CVSS 8.6). Enterprise management platforms continue to be top exploitation targets
- **AWS Security Agent multi-agent pentest** (February 2026): frontier AI agents for automated pentesting — specialized agents for recon, vulnerability analysis, and exploit validation working collaboratively. Now supports shared VPC and GitHub Enterprise Cloud. Increases automation pressure on standard vulnerability classes
- **PromptArmor plugin hook injection** (March 2026): Claude Code marketplace plugins inject prompts via exit-2 stderr channel → agent follows as system instructions → demonstrated full codebase exfiltration. Generalizable to any AI tool with plugin hook systems lacking source attribution
- **XBOW APAC expansion** (January 2026): appointed WonLae Lee (ex-Samsung SDS Red Team) as GM South Korea. APAC expansion planned throughout 2026. Ron Gabrisko (Databricks CRO) joined board. Competitive pressure increasing in all geographies
- **Salesforce bounty record year** (March 2026): **$6.2M paid in 2025** — program's most productive year; 696 hackers participated; individual payouts up to $60K; $30.4M total since 2015. Demonstrates mature enterprise programs paying more per finding as AI handles low-hanging fruit
- **HackerOne Agentic PTaaS** (January 2026): continuous pentesting combining AI agents with human experts; **88% accuracy** on fix-verified findings; trained on proprietary exploit intelligence. First major platform offering hybrid AI+human pentesting as a service
- **Gogs LFS supply chain risk** (March 2026): CVE-2026-25921 (CVSS 9.3) — cross-repository LFS object overwrite without auth. Replace legitimate binaries with backdoored payloads. Git-based artifact registries as supply chain attack vectors
- **MCPTox benchmark** (arXiv:2508.14925): first systematic benchmark of tool poisoning — 45 live MCP servers, 353 tools; **72.8% attack success** (o1-mini); more capable models MORE susceptible; <3% refusal rate. Quantifies MCP tool poisoning as a systemic risk, not theoretical
- **Cisco SD-WAN active exploitation** (March 2026): CVE-2026-20128/20122 actively exploited; web shell deployment confirmed; ACSC + CISA advisories. Combined with CVE-2026-20127 (CVSS 10.0) makes SD-WAN infrastructure a top exploitation target
