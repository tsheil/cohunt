...
- **LeakyLooker** (Tenable, March 10 2026): 9 cross-tenant vulnerabilities in Google Looker Studio — report cloning preserves original owner's database credentials; one-click data exfiltration via shared reports forces victim's browser to proxy queries against attacker-controlled project; affected BigQuery, Spanner, PostgreSQL, MySQL, Sheets, GCS connectors. All fixed by Google after June 2025 responsible disclosure. **Hunter implication:** cloud analytics/BI tools (Looker Studio, Power BI, Tableau Cloud, Metabase) that share reports with embedded service credentials are systematic cross-tenant attack surface — test report cloning, sharing, and embedding for credential inheritance
- **Equixly agentic pentesting benchmark** (March 2026): Equixly's Agentic AI Hacker identified **230 vulnerabilities in 1 hour** across 30 challenges; 15 human pentesters solved 14/30 challenges in 2+ hours; AI found **80% more issues than ZAP DAST** and 8 high-severity vulns DAST missed entirely; 43% of tested popular MCP servers had command injection. **Hunter implication:** AI pentesting tools are now definitively outperforming traditional DAST and matching junior human pentesters on coverage — human competitive advantage is business context, multi-step chain reasoning, and novel attack patterns
- **Microsoft March 2026 Patch Tuesday** (March 11, 2026): 84 CVEs total, 8 Critical; includes SharePoint RCE pair (CVE-2026-26106 unauth + CVE-2026-26114 deser, CVSS 8.8), SQL Server zero-day EoP (CVE-2026-21262 publicly disclosed), AFD.sys EoP (CVE-2026-24293 → SYSTEM), Zoom Workplace file path control (CVE-2026-30903 CVSS 9.6). Chain potential: SharePoint unauth RCE → SQL Server EoP → SYSTEM. **Hunter implication:** on-prem Microsoft infrastructure (SharePoint 2016/2019/SE, SQL Server, Windows Server) is a cluster of chainable vulns; test deserialization gadget chains on accessible SharePoint instances
