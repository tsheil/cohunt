...
- **GitHub Taskflow Agent** (March 6, 2026) — open-source structured security audit framework using LLMs; found **80+ vulnerabilities** in 40 open-source repos (~20 publicly disclosed) including WooCommerce CVE-2025-15033; excels at auth bypasses, IDORs, token leaks; requires GitHub Copilot license. **Hunter implication:** validates structured multi-step audit approach (similar to Cohunt's progressive disclosure); auth bypass and IDOR findings are human-edge targets
