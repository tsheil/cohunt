...
| CVE-2026-1721 | Cloudflare AI Playground | Reflected XSS in OAuth handler | Unescaped `error_description` interpolation — AI platform OAuth error handling pattern |
...
## 4. XS-Leaks, WebSocket Hijacking, & Service Worker Attacks

> **Specialist browser primitives** — full testing procedures in [reference/browser-apis.md](reference/browser-apis.md).

| Load When | What You Get |
|-----------|-------------|
| `wss://` URLs in proxy history | WebSocket CSWSH hijacking tests (CWE-346) |
| `navigator.serviceWorker.register()` in JS | Service worker scope abuse, cache poisoning (CWE-349) |
| Need cross-origin state detection without response read | XS-Leak techniques: frame counting, timing, ETag length leak, connection pool (CWE-203) |
| SameSite=None cookies on target | Most XS-Leak vectors become viable |
...
7. SPA-specific: Test route guards, hydration data, source maps
8. Browser APIs: If WS/SW/XS-Leak indicators present → load browser-apis.md
9. Escalation: Chain findings (see chaining matrix below)
```
...
|-----------|------------|----------|----------|
| Prototype pollution | innerHTML/template gadget | XSS (arbitrary JS) | High |
| DOM XSS (reflected) | Admin panel context | Stored XSS in admin | Critical |
| PostMessage no origin check | Token in response | Account takeover | Critical |
| CORS reflected origin | Authenticated endpoint | Cross-origin data theft | High-Critical |
| CSP bypass (`unsafe-inline`) | Reflected injection point | Full XSS despite headers | High |
| Source map exposure | Hardcoded API key in source | Key theft + API abuse | High |
| PP + sanitizer bypass | Stored context | Persistent XSS | Critical |
| Client-side route bypass | Server also missing auth | Unauthorized access | High |
...
| File | Contents | Lines |
|------|----------|-------|
| [reference/browser-apis.md](reference/browser-apis.md) | XS-Leak techniques (frame counting, timing, ETag length, connection pool), WebSocket hijacking (CSWSH), service worker scope abuse and cache poisoning | ~95 |

**Quick search** — find specific browser API patterns:
```
grep -n "XS-Leak\|frame counting\|timing\|ETag\|connection pool" ${CLAUDE_SKILL_DIR}/reference/browser-apis.md
grep -n "WebSocket\|CSWSH\|Origin\|hijack" ${CLAUDE_SKILL_DIR}/reference/browser-apis.md
grep -n "service worker\|scope\|cache poisoning\|SW" ${CLAUDE_SKILL_DIR}/reference/browser-apis.md
```
